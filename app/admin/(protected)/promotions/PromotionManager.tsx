...
import type { CampaignType, Promotion, PromotionScope } from '@/lib/promotions'
...
export default function PromotionManager({
  ...
  function edit(promotion: Promotion) {
    ...
    setForm({
      ...
      discount_percent: promotion.discount_percent == null ? '10' : String(promotion.discount_percent),
    ...
    <div className="grid gap-10 xl:grid-cols-[minmax(20rem,30rem)_1fr] xl:items-start">
      <form onSubmit={submit} className="border border-line bg-paper-raised p-5 sm:p-6 xl:sticky xl:top-6">
        <div className="mb-6 flex items-start justify-between gap-4">
          <div>
            <p className="t-meta">{editingId ? 'Editing campaign' : 'New campaign'}</p>
        ...
        <div className="space-y-5">
          <div>
            <label htmlFor="promotion-name" className={label}>Campaign name</label>
          ...
          <fieldset>
            ...
            <div className="grid grid-cols-2 gap-2">
              {(['event', 'discount'] as const).map((campaignType) => (
                <button
                  key={campaignType}
                  type="button"
                  aria-pressed={form.campaign_type === campaignType}
                  onClick={() => setForm((current) => ({
                    ...current,
                    campaign_type: campaignType,
                    scope: campaignType === 'event' ? 'sitewide' : current.scope,
                    category_id: campaignType === 'event' ? '' : current.category_id,
                  }))}
                  className={`min-h-11 border px-3 text-sm ${form.campaign_type === campaignType ? 'border-ink bg-ink text-paper' : 'border-line text-ink-dim'}`}
                >
                  {campaignType === 'event' ? 'Event / announcement' : 'Discount sale'}
                </button>
              ))}
            </div>
          </fieldset>
          ...
          <label className="flex min-h-11 cursor-pointer items-center gap-3">
            ...
            <span className="text-xs text-ink-dim">Only enabled campaigns can go live.</span>
          </label>
        </div>

        <button type="submit" disabled={saving} className="btn btn-primary mt-6 w-full">
          {saving ? 'Saving…' : editingId ? 'Save changes' : `Create ${form.campaign_type}`}
        </button>
      </form>
      ...
      <section aria-labelledby="campaign-list-heading">
        ...
        {sorted.length === 0 ? (
          <div className="border border-dashed border-line px-5 py-16 text-center text-sm text-ink-dim">No campaigns yet. Create the first event or discount.</div>
        ) : (
                  ...
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <span className="border border-line px-2 py-1 text-[0.6875rem] uppercase tracking-wider text-ink-dim">{promotion.campaign_type}</span>
                        <span className={`px-2 py-1 text-[0.6875rem] ${status.className}`}>{status.label}</span>
                      </div>
                      {promotion.campaign_type === 'discount' && promotion.discount_percent != null ? (
                        <p className="mt-2 text-sm text-ink">
                          <strong className="tnum font-medium">{Number(promotion.discount_percent).toLocaleString()}% off</strong>
                          {' · '}{promotion.scope === 'sitewide' ? 'Entire site' : promotion.category?.name ?? 'Selected category'}
                        </p>
                      ) : (
                        <p className="mt-2 text-sm text-ink-dim">Site-wide storefront announcement</p>
                      )}
