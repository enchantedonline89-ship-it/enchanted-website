...
export default function DeleteProductButton({ id }: { id: string; name: string }) {
  ...
  const handleDelete = async () => {
    ...
    setError(null)
    try {
      await adminCatalogRequest(`/api/admin/products/${id}`, { method: 'DELETE' })
      router.refresh()
    } catch (err) {
  ...
  if (confirm) {
    return (
      <div className="flex flex-wrap items-center gap-2">
        <span className="text-signal-error text-xs">Hide this product?</span>
